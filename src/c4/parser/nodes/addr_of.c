...
c4_ast_node_addr_of_make(
    ...
    self->span = span;
    self->free_func = c4_ast_node_addr_of_free;
    self->to_str_func = c4_ast_node_addr_of_to_str;

    self->addr_of.expr = expr;
...
c4_ast_node_addr_of_to_str(
    ...
    c4_sb_kvp_start(sb, c4_strv_lit("addr_of"));
    {
        c4_sb_kvp_append_key(sb, c4_strv_lit("expr"));
        c4_sb_line_end(sb);

        c4_sb_kvp_value_start(sb);
        c4_ast_node_to_str(self->addr_of.expr, sb);
...
c4_ast_node_addr_of_parse(
    ...
    c4_token ampersand = c4_parser_ctx_pop_expect(ctx, c4_token_ampersand);
    
    c4_ast_node* expr = c4_ast_node_expr_parse(ctx);

    return c4_ast_node_addr_of_make(ampersand.span, expr);
}
