...
c4_ast_node_dtype_make(
    ...
    self->span = span;
    self->free_func = c4_ast_node_dtype_free;
    self->to_str_func = c4_ast_node_dtype_to_str;
...
c4_ast_node_dtype_to_str(
    ...
    c4_sb_kvp_start(sb, c4_strv_lit("dtype"));
    {
        c4_sb_kvp_append_key(sb, c4_strv_lit("name"));
        
        c4_sb_kvp_value_start(sb);
        {
            c4_sb_append_str(sb, self->dtype.name);
        }
